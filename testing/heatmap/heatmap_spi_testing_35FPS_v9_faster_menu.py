...
# optional: reuse these buffers to reduce allocs
_tmp_w = None
_tmp_out = None
...
# ===============================================================
# 11.6 SPI "latest fft_data" thread (NEW)
# ===============================================================
class _LatestSpiData:
    def __init__(self):
        self.lock = threading.Lock()
        self.fft_data = None          # np.ndarray (N_MICS, N_BINS) complex64
        self.ok = False
        self.frame_id = 0
        self.last_err = ""
        self.frames_ok = 0
...
def _spi_worker():
    ...
    # Ensure clean counters in mailbox
    with _spi_latest.lock:
        _spi_latest.ok = False
        _spi_latest.fft_data = None
        _spi_latest.frame_id = 0
        _spi_latest.last_err = ""

    while not _stop_spi:
        try:
            fr = spi_source.read_frame()   # <-- your existing heavy/blocking function

            # Mirror stats out of spi_source (so UI can still show them)
            with _spi_latest.lock:
                _spi_latest.last_err = spi_source.last_err
                _spi_latest.frames_ok = spi_source.frames_ok
                _spi_latest.bad_parse = spi_source.bad_parse
                _spi_latest.bad_crc = spi_source.bad_crc
                _spi_latest.sclk_hz_rep = spi_source.sclk_hz_rep

                if fr is not None and fr.fft_data is not None:
                    # IMPORTANT: make sure we store a stable array
                    # (fr.fft_data is already a new array each read_frame, so this is fine)
                    _spi_latest.fft_data = fr.fft_data
                    _spi_latest.frame_id = getattr(fr, "frame_id", _spi_latest.frame_id + 1)
                    _spi_latest.ok = True

        except Exception as e:
            with _spi_latest.lock:
                _spi_latest.last_err = f"spi_thread_exc:{e}"
                _spi_latest.bad_parse = spi_source.bad_parse
            time.sleep(0.002)  # avoid spinning if something is wrong


def start_spi_thread():
    global _spi_thread, _stop_spi
    if _spi_thread is not None and _spi_thread.is_alive():
        return
    _stop_spi = False
    _spi_thread = threading.Thread(target=_spi_worker, daemon=True)
    _spi_thread.start()

def stop_spi_thread():
    global _stop_spi, _spi_thread
    _stop_spi = True
    time.sleep(0.01)
    _spi_thread = None
    ...
    try:
        while True:
            ...
            if button_state.source_mode == "SIM":
                ...
                # Non-blocking: just grab latest
                with _spi_latest.lock:
                    spi_fft = _spi_latest.fft_data
                    source_err = _spi_latest.last_err

                    # mirror stats into spi_source for your existing UI text (optional)
                    spi_source.frames_ok = _spi_latest.frames_ok
                    spi_source.bad_parse = _spi_latest.bad_parse
                    spi_source.bad_crc = _spi_latest.bad_crc
                    spi_source.sclk_hz_rep = _spi_latest.sclk_hz_rep

                if spi_fft is not None:
                    # Make a minimal FFTFrame-like object, or just pass fft_data directly downstream.
                    # Lowest-change option: keep your existing fft_frame workflow:
                    f = FFTFrame()
                    f.channel_count = N_MICS
                    f.sampling_rate = SAMPLE_RATE_HZ
                    f.fft_size = SAMPLES_PER_CHANNEL
            ...
            # --- Blend heatmap left (FAST: LUT weights + integer blend) ---
            left_bg = base_frame[:, :left_width, :]                         # view (no copy)
            colored = cv2.applyColorMap(heatmap_left, cv2.COLORMAP_MAGMA)   # uint8 (HxWx3)

            # weight per pixel (HxW) in 0..255
            w = cv2.LUT(heatmap_left, W_LUT_U8)                             # uint8 (HxW)

            # Expand to 3 channels without np.stack (still allocs but much lighter than float path)
            w3 = cv2.merge((w, w, w))                                       # uint8 (HxWx3)

            # out = (colored*w + bg*(255-w) + 127) >> 8   (integer blend)
            inv = cv2.bitwise_not(w3)                                       # 255 - w3

            # Use OpenCV mul/add to stay in C++ (fewer Python-side temporaries)
            c1 = cv2.multiply(colored, w3, scale=1/255.0)                   # uint8-ish result
